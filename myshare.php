...
.pbl {
	width: '.$pbl_w.'px;
				...
				<h3>收藏夹</h3>
				<div class="sub_title">
					<span>好友的分享</span>|<a href="myfav.php">我喜欢的</a>
				</div>
			</div>
			<div class="act_cont clearfix">
				<div style="height: 20px;"></div>';
$q_rep=sprintf('select a.*, b.xqid, b.hzid from %s as a, %s as b, %s as c, %s as d where a.is_del=0 and a.jlid=b.jlid and a.jpid=c.jpid and c.uid=d.buddyid and c.tlid=2 and d.uid=%s order by c.datetime desc limit %s', $yjl_dbprefix.'jl_photo', $yjl_dbprefix.'jl', $yjl_dbprefix.'jl_plike', $dbprefix.'buddys', $udb['uid'], ($pbl_p*$pbl_s));
	...
	$js_scrc='
		...
		if(clienth+scrollt+200>scrollh){
			if($(\'#isloading\').val()==\'0\'){
				...
				p++;
				$.get(\'j/myshare.php?p=\'+p+\'&s=\'+$(\'#sp\').val(), function(data){
					if(data!=\'\'){
						$(\'#pbl_pv\').append(data);
						$("div[jq_newh=\'1\']").each(function(){
							var id=$(this).attr(\'jq_id\');
							if($("div[jq_id=\'"+id+"\']").length>1){
								$(this).remove();
							}else{
								$(this).fadeIn(500);
								$(this).attr(\'jq_newh\', \'0\');
