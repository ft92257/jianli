...
if($udb['uid']>0){
	$q_res=sprintf('select zz_adimg from %s where %sid=%s and uid=%s limit 1', $yjl_dbprefix.$mdb, $mdb, $id, $user_id);
	$res=mysql_query($q_res) or die('');
	$r_res=mysql_fetch_assoc($res);
			...
			if(is_uploaded_file($f_i['tmp_name']) && $f_i['error']==0){
				...
				if($e==0){
					...
					if(@copy($f_i['tmp_name'], $photo)){
						...
						if($data && $data[2]<=3){
							if($r_res['zz_adimg']!='' && file_exists('../'.$r_res['zz_adimg']))unlink('../'.$r_res['zz_adimg']);
							$uSQL=sprintf('update %s set zz_adimg=%s where %sid=%s', $yjl_dbprefix.$mdb, yjl_SQLString($u_f.$f_m.'.'.$f_e, 'text'), $mdb, $id);
