...
$q_res=sprintf('select a.*, b.hzid, b.uid as jluid, b.app_id, b.xqid, b.name as jlname from %s as a, %s as b where a.clid=%s and a.jlid=b.jlid', $yjl_dbprefix.'jl_cl', $yjl_dbprefix.'jl', $id);
...
if(mysql_num_rows($res)>0){
	...
	if($user_id>0 && isset($_POST['c']) && trim($_POST['c'])!=''){
		...
		if(!isset($jsg_result['error']) && isset($jsg_result['result']['tid'])){
			...
			$oid=0;
			if($user_id==$r_res['jluid']){
				$oid=2;
				$uSQL=sprintf('update %s set oid=0 where uid=%s and clid=%s', $yjl_dbprefix.'jl_topic', $user_id, $r_res['clid']);
				$result=mysql_query($uSQL) or die('');
	...
	echo yjl_cltopic($r_res['clid'], $r_res['hzid'], $page);
}
