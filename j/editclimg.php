...
if(isset($_POST['cookie_auth']) && trim($_POST['cookie_auth'])!=''){
	...
	$id=(isset($_POST['clid']) && intval($_POST['clid'])>0)?intval($_POST['clid']):1;
}else{
	$udb=yjl_chkulog();
	$id=(isset($_GET['id']) && intval($_GET['id'])>0)?intval($_GET['id']):1;
	if(isset($_POST['clid']) && intval($_POST['clid'])>0)$id=intval($_POST['clid']);
}
if($udb['uid']>0){
	$user_id=$udb['uid'];
	$adb=($udb['qx']==10 || $udb['isxg']>0)?'':' and a.uid='.$user_id;
	$q_res=sprintf('select a.clid, a.url, a.t_url, a.o_url, a.tid, a.datetime, a.uid from %s as a, %s as b where a.clid=%s and a.jlid=b.jlid%s limit 1', $yjl_dbprefix.'jl_cl', $yjl_dbprefix.'jl', $id, $adb);
	$res=mysql_query($q_res) or die(mysql_error());
	$r_res=mysql_fetch_assoc($res);
	if(mysql_num_rows($res)>0){
		if(isset($_FILES['Filedata']) && $_FILES['Filedata']['name']!=''){
				...
				if($e==0){
					...
					if(@copy($f_i['tmp_name'], $photo_o)){
						...
						if($data && $data[2]<=3){
							...
							$uSQL=sprintf('update %s set url=%s, t_url=%s, o_url=%s, width=%s, height=%s where clid=%s', $yjl_dbprefix.'jl_cl',
							...
							if(isset($_POST['is_nu']) && $_POST['is_nu']==1){
								echo '<script type="text/javascript">window.parent.upimg_a_9(\''.$u_f.$f_m.'.'.$f_e.'|'.$u_f.$f_m.'_t.'.$f_e.'|'.$u_f.$f_m.'_o.'.$f_e.'|'.$sw.'|'.$sh.'\');window.parent.hidenuv(\'5\');</script>';
								exit();
							}else{
			...
			$uSQL=sprintf('update %s set content=%s where clid=%s', $yjl_dbprefix.'jl_cl', yjl_SQLString($content, 'text'), $r_res['clid']);
			$result=mysql_query($uSQL) or die(mysql_error());
			echo $content;
			exit();
		}
