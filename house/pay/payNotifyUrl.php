...
require_once ("./tenpay_config.php");
	...
	//如果有使用折扣券，discount有值，total_fee+discount=原请求的total_fee
				...
				if (empty($aPay) || $aPay['result'] != 0) {
					...
					if ($aPay['type'] == 1) {
						$oDb->query("UPDATE tb_user SET money = money + " . ((float)$total_fee/100) . " WHERE id = " . $aPay['id'];
					} elseif ($aPay['type'] == 2) {
						$oDb->query("UPDATE tb_manager SET money = money + " . ((float)$total_fee/100) . " WHERE id = " . $aPay['id'];
					}
					//TODO 事务处理
				}
				//处理数据库逻辑
