...
function calculate(){
	$.post(URL+'/calculate', {acreage:$('#acreage').val(), budget:$('#budget').val(), grade:$('#grade').val()}, function(json){
		...
		<?php if(is_array($fields)): foreach($fields as $field=>$vo): ?>var scale_<?php echo ($field); ?> = Math.round(data.design/total);
			...
			$("#pie_image").attr("src", "<?php echo U('pie');?>/data/" + strData);
