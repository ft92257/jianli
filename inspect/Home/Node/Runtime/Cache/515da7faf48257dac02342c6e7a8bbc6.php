...
<body>
	<div class="page-bg">
    	<h1 class="h-title-s pt-9 ml-2 mr-5-5 align-c"><?php echo ($node[name]); ?>阶段，验收点</h1>
        <div class="bd" id="scroll_bd">
            <div class="top" id="bd">
                <ul id="ul" class="list-li">
                <?php if(is_array($acceptance)): foreach($acceptance as $key=>$vo): ?><li><a href="javascript:;" data-id="<?php echo ($vo['id']); ?>" class="acceptance_info"><?php echo ($vo[name]); ?>&nbsp;（<?php echo ($vo[aceptance_no_c]+$vo[aceptance_pass_c]+$vo[aceptance_nopass_c]); ?>）</a></li><?php endforeach; endif; ?>
                </ul>
            </div>
		</div>
    </div>
...
var GROUP= "__GROUP__"
$(function(){
	$(".acceptance_info").bind('click', function(){
		 parent.$.layer({
			    type: 2,
			    maxmin: true,
			    shadeClose: true,
			    ...
			    offset: ['20px',''],
			    area: ['804px', '702px'],
			    iframe: {src: GROUP+'/Acceptance/info/case/'+$(this).attr('data-id')}
			}); 
		});
});
</script>
