...
function selectNewNav(id, curClass) {
	if (!$("#"+id).length) {
		return false;
	}
...
$(function(){
	selectNav('main_menu', 'mmoren');
	selectNewNav('main_menu_new', 'mrck');
});
...
function showReport(bShow) {
	...
	if (bShow) {
		...
		$("#div_comment").hide();
		arr[0].className="";
		arr[1].className="bg_color";
	} else {
		$("#form_report").hide();
		$("#div_comment").show();
		arr[0].className="bg_color";
		arr[1].className="";
	}
}
 ...
 * 控制下一级是否显示
 */
function selectShowTarget(field, index, ids) {
	var objs = ids.split(',');
...
$(function(){
	$(".menu>a").hover(function(){
		var indeaIndex=$(this).index();
		if (parent != window) {
			var left=$(this).position().left;
			var top=$(this).position().top + 3;
			var obj = $(".show").eq(indeaIndex);
			var s = "<div class='"+obj[0].className+"'>" + obj.html() + "</div>";
			//alert(left);
			parent.showMenu(s ,left,top);
		}else{
		  currentMenu = $(this);
		  $(".show").eq(indeaIndex).show().siblings(".show").hide();
		  $(".show").eq(indeaIndex).css("left",$(this).position().left);
		  $(".show").eq(indeaIndex).css("top",$(this).position().top + 95);
		} },function(){
			  $(".show").hide();
			  
			 });
	$(".menu .show").mouseover(function(){
		$(this).show();
		$(this).css("left", currentMenu.position().left - 10);
