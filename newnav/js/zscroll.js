...
$("p[class = nv1]").css('backgroundPosition', 'left -45px');
	...
	if($("#snav").length>0){
		...
		$(window).bind('scroll',function(){
			...
			}else if(s >= move5 && s < move6){
				a([1,2,3,4,6]);
				$("p[class = nv5]").css('backgroundPosition', '-180px -45px');
				$("p[class = nv5]").next("strong").css({'background':'#EB7C49','color':'#fff'});
			}else if(s >= move6){
				a([1,2,3,4,5]);
				$("p[class = nv6]").css('backgroundPosition', '-225px -45px');
				$("p[class = nv6]").next("strong").css({'background':'#EB7C49','color':'#fff'});
			}
